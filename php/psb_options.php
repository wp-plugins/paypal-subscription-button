...
if( !class_exists( 'psb_Options' ) ) {
	...
	class psb_Options {
		...
		function get_psb_options() {
			 ...
			 if( !empty( $aon ) && ( !isset( $this->post_vars[ 'updatepsboptions' ] ) || !$this->post_vars[ 'updatepsboptions' ]  ) ) {
			 ...
			 if( isset( $this->post_vars[ 'updatepsboptions' ] ) && true == $this->post_vars[ 'updatepsboptions' ] ) {
		...
		function create_notify_page( &$aon, &$concat_admin_options_name ) {
			...
			if( empty( $aon[ 'autoset_ipn_page_ID' ] ) && 1 == $concat_admin_options_name[ 'autoset_ipn_page' ] ) {
				//Creates page data
				global $current_user;
