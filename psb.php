...
if( !class_exists( 'psb_PSB' ) ) {
	...
	class psb_PSB {
		...
		function listener() {
			...
			if( is_page( $page_id ) ) {
				...
				if ( $this->psb_ipn->is_verified() ) {
					...
					if( $this->is_empty_string( $payment_status ) && $this->is_empty_string( $txn_type ) ) {
		...
		function process_completed() {
			...
			if( $receiver_email != $this->paypal_email || $mc_currency != $this->currency ) {
				return "Someone attempted a sale using a manipulated URL"; 	
			}
			
			// get final option. e.g. monthly
			...
			if( $this->is_empty_string( $payment_type ) || $this->is_empty_string( $membership_type ) ) {
				return "NULL payment_type & membership_type";
			}
													
			if( $this->psb_query->transact_id_exists() || $this->psb_query->wp_user_id_exists() ) { 
			...
			if( !$trans_reg_result || !$subscr_reg_result ) {
				return "This was NOT a successful transaction due to database query issues.";
			}
			
			//set the user's role to $membership_type. e.g. gold or silver			
		...
		function process_end( $subscr_status ) {
			...
			if( $this->psb_query->update_status( $subscr_status ) && $this->psb_query->log_end() ) { 
